...
console.log(`${colors.bright}${colors.blue}🔧 Inyectando variables de entorno para Vercel...${colors.reset}\n`);

// Leer variables de entorno
const API_BASE_URL = process.env.NG_APP_API_BASE_URL || 'https://api-ecommerce-d9fxeccbeeehdjd3.eastus-01.azurewebsites.net';
const VAPID_PUBLIC_KEY = process.env.NG_APP_VAPID_PUBLIC_KEY || 'REPLACE_WITH_YOUR_VAPID_PUBLIC_KEY';
...
console.log(`${colors.green}✓${colors.reset} Entorno detectado: ${IS_VERCEL ? 'Vercel' : 'Local'} (${VERCEL_ENV})`);
console.log(`${colors.green}✓${colors.reset} API Base URL: ${colors.bright}${API_BASE_URL}${colors.reset}`);
console.log(`${colors.green}✓${colors.reset} Log Level: ${LOG_LEVEL}`);
console.log(`${colors.green}✓${colors.reset} Analytics: ${ENABLE_ANALYTICS ? 'Habilitado' : 'Deshabilitado'}`);
...
const outputPath = path.join(__dirname, '..', 'apps', 'pwa', 'src', 'environments', 'environment.runtime.ts');
...
if (!process.env.NG_APP_API_BASE_URL) {
  console.log(`${colors.yellow}⚠️  NG_APP_API_BASE_URL no definida, usando valor por defecto${colors.reset}`);
}

if (VAPID_PUBLIC_KEY === 'REPLACE_WITH_YOUR_VAPID_PUBLIC_KEY') {
  console.log(`${colors.yellow}⚠️  NG_APP_VAPID_PUBLIC_KEY no definida, usando placeholder${colors.reset}`);
}
