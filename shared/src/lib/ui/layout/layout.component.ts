...
export class LayoutComponent implements OnInit {
  ...
  displayName = computed(() => this.currentTenant()?.displayName || 'PWA Store');
  ...
  private applyGlobalTenantTheme(tenant: any): void {
    ...
    if (tenant.branding) {
      ...
      // Core colors
      this.renderer.setStyle(root, '--tenant-primary-color', branding.primaryColor || '#3b82f6');
      this.renderer.setStyle(root, '--tenant-secondary-color', branding.secondaryColor || '#64748b');
      this.renderer.setStyle(root, '--tenant-accent-color', branding.accentColor || '#ef4444');
      this.renderer.setStyle(root, '--tenant-background-color', branding.backgroundColor || '#ffffff');
      this.renderer.setStyle(root, '--tenant-text-color', branding.textColor || '#1f2937');

      // Derived colors for consistency
      this.renderer.setStyle(root, '--tenant-primary-hover', this.darkenColor(branding.primaryColor || '#3b82f6', 0.1));
      this.renderer.setStyle(root, '--tenant-border-color', this.adjustColorOpacity(branding.textColor || '#1f2937', 0.2));
      this.renderer.setStyle(root, '--tenant-breadcrumb-bg', this.adjustColorOpacity(branding.backgroundColor || '#ffffff', 0.95));
    }
